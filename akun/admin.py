...
class UserAdmini(UserAdmin):
    ...
    list_display = ['username','is_active','is_pegawaiadmin', 'is_dokter', 'is_perawat']
    search_fields = ['username']
    list_per_page = 5
    fieldsets = (
        (('Personal info'), {'fields': ('username',)}),
        (('Permissions'), {'fields': ('is_active', 'is_staff', 'is_superuser', 'groups', 'is_pegawaiadmin', 'is_dokter', 'is_perawat')}),
        (('Important dates'), {'fields': ('date_joined', 'last_login')}),
    )
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'password1', 'password2', 'is_pegawaiadmin', 'is_dokter', 'is_perawat'),
        }),
    )
